...
const planterPlanteBackend = async (
  idLot,
  idPlanteur,
  idPlante,
  datePlantation,
  ...
  await BDAlimenthe.planterPlante(
    idLot,
    idPlanteur,
    idPlante,
    datePlantation,
...
const DeleteMembreBackEnd = async (idMembre) => {
  await BDAlimenthe.DeleteMembre(idMembre);
};

const DeleteJardinBackend = async (idJardin) => {
  await BDAlimenthe.DeleteJardin(idJardin);
};
...
const afficherMembresBackend = async () => {
  const membres = await BDAlimenthe.afficherMembres();
  return membres;
};
const afficherJardinBackend = async () => {
  const jardin = await BDAlimenthe.afficherJardin();
  return jardin;
};
const afficherPlantesBackend = async () => {
  const plantes = await BDAlimenthe.afficherPlantes();
  return plantes;
};
const afficherLotsBackend = async () => {
  const Lot = await BDAlimenthe.afficherLots();
  return Lot;
};
...
export default {
  ...
  addToLotBackend,
  addPlanteBackend,
  planterPlanteBackend,
  DeleteMembreBackEnd,
  DeleteJardinBackend,
  DowngradeAdminBackend,
  DeleteLotBackend,
  DeleteDemandeAttenteBackend,
  deletePlanteBackend,
  recolterPlanteBackend,
  afficherMembresBackend,
  afficherPlantesBackend,
  afficherLotsBackend,
  afficherPlantesLotsBackend,
  afficherJardinBackend,
};
